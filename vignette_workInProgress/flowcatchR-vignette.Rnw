...
Of course at any point of the analysis, the user may wish to export the images stored in a FrameList object. This can be done via the export.frames function, which offers multiple ways to export - animated gif or jpeg, pi pa pu []
