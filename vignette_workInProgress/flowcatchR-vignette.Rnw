...
allChannelsMesenterium <- createChannelsFrameList(MesenteriumSubset)
...
particlesPlatelets <- extractParticles(allChannelsMesenterium$red, # use just red channel
                                       preprocessedPlatelets)


# the same is achievable through 
particlesPlatelets <- extractParticles(allChannelsMesenterium$red)
...
# initialization
fullData <- newFrameList(nframes=100,imgsLocation=imgRepository)
...
\Robject{nframes} specifies the number of frames that will constitute the FrameList object, whereas \Robject{imgsLocation} is a vector of character strings with the location of the (raw) images. In this case we just loaded the full dataset, but for the demonstrational purpose of this vignette, we will proceed with the subset available in the MesenteriumSubset.RData object, which we previously loaded in Section \ref{sec:overview}.
...
For a detailed explanation of the parameters to better tweak the performances of this segmentation step, please refer to the help of preprocess.FrameList
...
As it can be seen from the summary information, each ParticleList ...

Denoising, registration, cut/rotate 
combinePrepro
particleExtr, filtering
