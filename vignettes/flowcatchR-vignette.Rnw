...
This document offers an introduction and overview of the \R{}/\Bioconductor{} \cite{Rlang2013,Gentleman2004} package \flowcatchR, which provides a set of tools to detect and track cells in time-lapse microscopy.
...
\begin{itemize}
\item Images come from intravital microscopy experiments. This means, SNR is not optimal, and very importantly there are potential major movements of the alive specimen, that can be confounded with the true movements of the particles of interest \cite{Meijering2008}
...
\item An algorithm for tracking the particles, adapted and improved from the proposal of Sbalzarini and Koumoutsakos (2005) \cite{Sbalzarini2005} \TODO{ fix how to use refs with knitr and bioc vignette}, that reflects the directional aspect of the motion
\item A set of functions inspecting the kinematic properties of the identified trajectories \cite{Beltman2009,Meijering2012a}, providing summary statistics and visualization tools to help classifying identified objects
\end{itemize}
