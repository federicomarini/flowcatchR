...
\flowcatchR{} provides an S4 class that can store the information of a complete acquisition, namely \Rclass{Frames}. A \Rclass{Frames} object extends the \Rclass{Image} class, defined in the \Biocpkg{EBImage} package, and thus exploits the multi-dimensional array structures of the class. The locations of the images are stored as \Rfunction{dimnames} of the \Rclass{Frames} object. To construct a \Rclass{Frames} from images, the \Rfunction{read.Frames} function is used:
...
reimportedPlatelets <- read.particles(particle.files = "/path/to/export/folder/exportParticleSet/")
