...
inspect.frames(MesenteriumSubset,nframes=9,display.method="raster",inspectAll=TRUE)
@
...
If called with the parameter mode set to \Robject{particles}, then it will automatically generate a \Rclass{FrameList} object, with the contours of all particles drawn around the objects that passed the segmentation (and filtering) step (Fig.\ref{fig:checkSelection}).
...
To allow for a thorough evaluation of the single trajectories, \Rfunction{export.frames} is a valid helper, as it creates single images corresponding to each frame in the \Rclass{FrameList} object. We first extract for example trajectory 11 (Fig.\ref{fig:traj11}) with this command:
