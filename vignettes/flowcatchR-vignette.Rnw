...
This document offers an introduction and overview of the \R{}/\Bioconductor{} \cite{Rlang2013,Gentleman2004} package \flowcatchR, which provides a flexible and comprehensive set of tools to detect and track flowing blood cells in time-lapse microscopy. 

\flowcatchR builds upon functionalities provided by the \Biocpkg{EBImage} package \cite{Pau2010}, and extends them in order to analyze time-lapse microscopy images. Unique characteristics of the datasets \flowcatchR is designed for are listed here:

\begin{itemize}
\item The images come from intravital microscopy experiments. This means, SNR is not optimal, and very importantly there are potential major movements of the alive specimen, that can be confounded with the true movements of the particles of interest \cite{Meijering2008}
...
# currently could be done via github, I guess
...
Now, to perform the linking of the particles, we use link.particles. Important parameters are L and r, named as in the original implementation. L is the maximum displacement in pixels that a particle is expected to have in two consecutive frames, and r is the value for the link range, i.e. the number of future frames to be considered for the linking (typically assumes values between 1 - when no occlusions are known to happen - and 3). An extended explanation of the parameters is in the documentation of the package.
...
export.frames offers multiple ways to export - animated gif (if \software{ImageMagick} is available and installed on the system) or multiple jpeg images.
...
{
  paintedTraj <- add.contours2(raw.frames=MesenteriumSubset,
                               binary.frames=preprocessedPlatelets,
                               trajectories=trajPlatelets,
                               mode="trajectories",
                               col="yellow",
...
If you would like to get access to it, you can email \email{marinif@uni-mainz.de}.
