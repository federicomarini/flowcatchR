...
\date{Edited: February 2, 2015; Compiled: \today}
...
A further integration are a number of Jupyter/IPython notebooks (\cite{Perez2007}), as a way to provide easy reproducibility as well as communication of results, by combining plain text, commands and output in single documents. The R kernel used on the back-end was developed by Thomas Kluyver (\url{https://github.com/takluyver/IRkernel}) These are available upon request?/at github? i do not want to make the package too fat - or exceed the 4mb threshold normally set in Bioconductor. 

% also report IPython notebooks here?
